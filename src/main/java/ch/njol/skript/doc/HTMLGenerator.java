...
public class HTMLGenerator {
	...
	private static final Comparator<? super SyntaxElementInfo<?>> annotatedComparator = (o1, o2) -> {
		...
		Name name1 = o1.type.getAnnotation(Name.class);
		Name name2 = o2.type.getAnnotation(Name.class);
		if (name1 == null)
			throw new SkriptAPIException("Name annotation expected: " + o1.type);
		if (name2 == null)
			throw new SkriptAPIException("Name annotation expected: " + o2.type);
	...
	private static <T> Iterator<SyntaxElementInfo<? extends T>> sortedAnnotatedIterator(Iterator<SyntaxElementInfo<? extends T>> it) {
		...
		while (it.hasNext()) {
			...
			if (item.type.getAnnotation(Name.class) == null && item.type.getAnnotation(NoDoc.class) == null) {
				Skript.warning("Skipped generating '" + item.type + "' class due to missing Name annotation");
				continue;
			}
			list.add(item);
		}
			...
			while (generate != -1) {
				...
				if (genType.equals("sections") || isDocsPage) {
					for (Iterator<SyntaxElementInfo<? extends Section>> it = sortedAnnotatedIterator(Skript.getSections().iterator()); it.hasNext(); ) {
						SyntaxElementInfo<? extends Section> info = it.next();
						assert info != null;
						boolean isEffectSection = EffectSection.class.isAssignableFrom(info.type);
						// exclude sections that are EffectSection from isDocsPage, they are added by the effects block above
						if ((isEffectSection && isDocsPage) || info.type.getAnnotation(NoDoc.class) != null)
							continue;
	...
	private String generateAnnotated(String descTemp, SyntaxElementInfo<?> info, @Nullable String page, String type) {
		...
		String ID = docId != null ? (docId != null ? docId.value() : null) : info.type.getSimpleName();
		// Fix duplicated IDs
		if (page != null) {
